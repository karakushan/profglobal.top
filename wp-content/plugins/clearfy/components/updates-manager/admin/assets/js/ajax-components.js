		...
		$('.factory-ajax-checkbox').on('change', function(ev) {
			...
			$.ajax({
				...
				success: function(response) {
					...
					if( !response || !response.success ) {
						if( response.data.error_message ) {
							$.wbcr_factory_templates_131.app.showNotice('Error: [' + response.data.error_message + ']', 'danger');
						}
						return false;
					}

					/*var noticeId = $.wbcr_factory_templates_131.app.showNotice('Settings successfully updated', 'success');
				...
				error: function(xhr, ajaxOptions, thrownError) {
					...
					$.wbcr_factory_templates_131.app.showNotice('Error: [' + thrownError + '] Status: [' + xhr.status + '] Error massage: [' + xhr.responseText + ']', 'danger');
				}
			});
		});
